...
## 11. 最终结论：D0 已穷尽，"加分"在本账户数学上不可能

用户提交 IV skew(INDUSTRY, self-corr 0.672) 后 Performance Comparison **仍减分**。
至此 code-核对测遍**所有**数据轴：IV skew(强但拥挤0.67)、IV VRP/level(不相关0.09但
弱0.14)、impliediee(0.67)、价值/微结构(拥挤0.67)、质量/成长/新闻/分析师/社媒/关系/
脚注(全<0.8)。改 setting(中性化/universe)也只改善 margin、消不掉拥挤。

**铁律**：账户 28 个 alpha 饱和了所有强 D0 轴。D0 上强信号必 ~0.67 相关(减分)、不相关
信号必 <0.8(到不了2.0)；组合多个不相关弱信号上限 ~1.2(√6×0.5)，过不了 2.0 门槛。
**"D0 + 非拥挤(加分)"在此账户数学上不可能。**

唯一出路（均需放宽某约束）：
1. **delay=1**：门槛降到 Sharpe 1.25，不相关弱信号(VRP/关系/社媒/质量)可组合达标且
