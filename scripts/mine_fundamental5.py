...
def W(sig): return f"zscore(ts_decay_linear(group_neutralize({sig}, subindustry), 6))"
SPECS={
 "vq_T1k":"rank(ts_rank(cashflow_op/cap, 40)) + rank(ts_rank(ebit/equity, 40))",
 "vq_w60":"rank(ts_rank(cashflow_op/cap, 60)) + rank(ts_rank(ebit/equity, 60))",
 "vq3":"rank(ts_rank(cashflow_op/cap, 40)) + rank(ts_rank(ebit/equity, 40)) + rank(ts_rank(sales/cap, 40))",
 "earn_ebit":"rank(ts_rank(ebit/cap, 40)) + rank(ts_rank(ebit/enterprise_value, 40))",
 "cf_roa":"rank(ts_rank(cashflow_op/cap, 40)) + rank(ts_rank(ebit/assets_curr, 40))",
 "book_cf":"rank(ts_rank(bookvalue_ps/close, 40)) + rank(ts_rank(cashflow_op/cap, 40))",
 "val_mom":"rank(ts_rank(cashflow_op/cap, 40)) + rank(-ts_rank(returns, 5))",
}
def selfcorr(aid):
    for _ in range(16):
