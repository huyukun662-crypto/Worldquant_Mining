...
# ts_backfill converts event-type fundamentals into a daily matrix
def bf(f): return f"ts_backfill({f}, 252)"
NI=bf("fnd6_nis"); SAL=bf("fnd6_sales"); AT=bf("fnd6_cptmfmq_atq"); OI=bf("fnd6_oiadps"); COGS=bf("fnd6_cogss")
SPECS={
 "earn_yield":W(f"rank({NI}/cap)"),
 "sales_yield":W(f"rank({SAL}/cap)"),
 "op_yield":W(f"rank({OI}/cap)"),
 "ev_ebit":W(f"rank({OI}/enterprise_value)"),
 "roa":W(f"rank({NI}/{AT})"),
 "gross_margin":W(f"rank(({SAL}-{COGS})/{SAL})"),
 "asset_turn":W(f"rank({SAL}/{AT})"),
 "accruals":W(f"rank(-1*({NI}-cashflow_op)/{AT})"),
 "earn_growth":W(f"rank(ts_delta({NI},66)/(abs({NI})+1))"),
 "value_comp":W(f"rank(ts_rank({NI}/cap,60)) + rank(ts_rank(cashflow_op/cap,60))"),
 "qual_comp":W(f"rank({NI}/{AT}) + rank(({SAL}-{COGS})/{SAL})"),
}
def selfcorr(aid):
    for _ in range(16):
