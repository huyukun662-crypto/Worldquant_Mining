...
TSOPS  = ["id","zscore","mean","delta","std","sum","rank"]      # interpretable only
...
SEEDS=[
 ("low_vol",      g1([T("returns",op="std",w=60,sign=-1.0)])),                 # low-volatility anomaly
 ("momentum",     g1([T("returns",op="sum",w=250,sign=1.0)])),                 # 12m momentum
 ("st_reversal",  g1([T("returns",op="sum",w=20,sign=-1.0)])),                 # short reversal (corr w/ existing)
 ("value_bp",     g1([T("bookvalue_ps","close")])),                            # book/price value
 ("earnings_ya",  g1([T("ebit","assets")])),                                   # earnings/assets
 ("gross_profit", g1([T("sales","assets")])),                                  # ~gross profitability
 ("illiquidity",  g1([T("returns",den="volume",op="std",w=20,sign=1.0)])),     # Amihud-ish illiquidity
 ("size",         g1([T("cap",sign=-1.0)])),                                   # small-cap
 ("cashflow_y",   g1([T("cashflow_op","assets")])),                            # cashflow yield
 ("investment",   g1([T("assets",op="delta",w=250,sign=-1.0)])),               # low asset growth
]
def rand_term():
...
LAMBDA=0.4   # complexity regularization strength
def fitness(r,g):
    if not r or not r.ok: return -9.0, False
    hard=[c for c in r.checks if c.get("result")=="FAIL"]
    sc=r.self_corr
    submit_ok=(len(hard)==0) and (sc is None or abs(sc)<0.5)   # submittable AND uncorrelated
...
def main():
    ...
    log.info(f"auth {cm.credentials.username}  GAv2(simple/reg/uncorr) seed={seed} pop={POP} gens={GENS}")
    out=REPO/"WQ_GA_D0_SIMPLE.json"; cache={}; hall=[]
