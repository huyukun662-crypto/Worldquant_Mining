...
# all MATRIX-type fundamental fields (usable directly)
NI="est_netprofit_adj"; SAL="sales"; AT="est_tot_assets"; EBIT="est_ebit"; EBITDA="est_ebitda"; GI="est_grossincome"; EQ="est_shequity"
SPECS={
 "earn_yield":W(f"rank({NI}/cap)"),
 "sales_yield":W(f"rank({SAL}/cap)"),
 "ebit_yield":W(f"rank({EBIT}/cap)"),
 "ev_ebit":W(f"rank({EBIT}/enterprise_value)"),
 "ev_ebitda":W(f"rank({EBITDA}/enterprise_value)"),
 "book_yield":W(f"rank(bookvalue_ps/close)"),
 "roa":W(f"rank({NI}/{AT})"),
 "roe":W(f"rank({NI}/{EQ})"),
 "gross_margin":W(f"rank({GI}/{SAL})"),
 "asset_turn":W(f"rank({SAL}/{AT})"),
 "low_leverage":W(f"rank(-1*debt_lt/{AT})"),
 "value_comp":W(f"rank({NI}/cap) + rank({EBIT}/enterprise_value) + rank(cashflow_op/cap)"),
 "qual_comp":W(f"rank({NI}/{AT}) + rank({GI}/{SAL})"),
}
def selfcorr(aid):
    for _ in range(16):
