"""Round 107: push SH > 1.69 on the close x park blend frontier.

Curve so far:
  close=1.0  park=2  -> SH 1.51 SC 0.46  (R102g)
  close=1.0  park=3  -> SH 1.61 SC 0.55  (R106f)
  close=0.5  park=2  -> SH 1.69 SC 0.63  (R106g)

Direction: less close-weight + more park-weight = higher SH/SC.

  R107a  close 0.3x + park 2x       push close down
  R107b  close 0.4x + park 2x       intermediate
  R107c  close 0.5x + park 2.5x     push park
  R107d  close 0.5x + park 2x d=10  faster decay
  R107e  close 0.5x + park 2x d=12  decay sweep
  R107f  close 0.5x + park C9 1+1   triple at light close
  R107g  close 0.3x + park 3x       both extremes
  R107h  close 0.5x + park 2x IND   broader neut
...
VARIANTS = [
    {"label": "R107a_c03_park2",         "expression": W(f"add({mulk(P27, 0.3)}, {mulk(PARK, 2)})", 15)},
    {"label": "R107b_c04_park2",         "expression": W(f"add({mulk(P27, 0.4)}, {mulk(PARK, 2)})", 15)},
    {"label": "R107c_c05_park25",        "expression": W(f"add({mulk(P27, 0.5)}, {mulk(PARK, 2.5)})", 15)},
    {"label": "R107d_c05_park2_d10",     "expression": W(f"add({mulk(P27, 0.5)}, {mulk(PARK, 2)})", 10)},
    {"label": "R107e_c05_park2_d12",     "expression": W(f"add({mulk(P27, 0.5)}, {mulk(PARK, 2)})", 12)},
    {"label": "R107f_c05_park_C9",       "expression": W(f"add({mulk(P27, 0.5)}, add({PARK}, {C9}))", 15)},
    {"label": "R107g_c03_park3",         "expression": W(f"add({mulk(P27, 0.3)}, {mulk(PARK, 3)})", 15)},
    {"label": "R107h_c05_park2_IND",     "expression": W(f"add({mulk(P27, 0.5)}, {mulk(PARK, 2)})", 15),
...
def main():
    ...
    session_id = f"{dt.datetime.now():%Y%m%d}_us_equity_r107_push_frontier_wq"
    session_dir = REPO / "logs" / session_id
    for sub in ("inputs", "working", "outputs"):
        (session_dir / sub).mkdir(parents=True, exist_ok=True)
    (session_dir / "inputs" / "objective.md").write_text(
        "R107: push SH > 1.69 on close-park blend frontier (SC < 0.7).\n"
    )
