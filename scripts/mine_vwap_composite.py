...
SPECS={
 # VWAP-deviation reversal (distinct price-volume mechanism vs corr)
 "vwap_rev":("zscore(ts_decay_linear(group_neutralize(-1*(close - vwap)/(vwap+0.001), subindustry), 6))","TOP3000","SUBINDUSTRY",0),
 "vwap_z":("zscore(ts_decay_linear(group_neutralize(-1*ts_zscore((close-vwap)/(vwap+0.001), 20), subindustry), 6))","TOP3000","SUBINDUSTRY",0),
 "vwap_av":("zscore(ts_decay_linear(group_neutralize(-1*ts_av_diff((close-vwap)/(vwap+0.001), 10), subindustry), 6))","TOP3000","SUBINDUSTRY",0),
 # dollar-volume shock (distinct from turnover z)
 "dvol_shock":("zscore(ts_decay_linear(group_neutralize(-1*ts_zscore(close*volume, 10), subindustry), 6))","TOP3000","SUBINDUSTRY",0),
 # COMPOSITE: pv-divergence(1.21) + vwap-reversal -> two orthogonal price-volume mechanisms, aim >1.25 clean
 "pv_plus_vwap":("zscore(ts_decay_linear(group_neutralize(add(-1*ts_corr(close,volume,5), -1*ts_zscore((close-vwap)/(vwap+0.001),20)), subindustry), 6))","TOP3000","SUBINDUSTRY",0),
 # COMPOSITE: pv-divergence + dollar-volume shock
 "pv_plus_dvol":("zscore(ts_decay_linear(group_neutralize(add(-1*ts_corr(close,volume,5), -0.5*ts_zscore(close*volume,10)), subindustry), 6))","TOP3000","SUBINDUSTRY",0),
}
def realsc(aid):
    for _ in range(12):
    ...
    while time.time()-t0<3300:
        ...
        if st=="COMPLETE":
            ...
            if sh>=1.25 and not nf and isinstance(rc,float) and rc<0.7:
                comp=" & 2020+" if isinstance(y20,(int,float)) and y20>0.3 else ""
                tag=" **VWAP-CLEAN(corr%.3f,2020=%s%s)**"%(rc,y20,comp)
