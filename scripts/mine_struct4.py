...
SIG={
 "accel":"(-1 * ts_delta(ts_delta(close, 5), 5))",
 "skew3":"(-1 * ts_mean(signed_power(returns, 3), 20))",
 "cv":"(-1 * ts_std_dev(close, 20) / (ts_mean(close, 20) + 0.001))",
 "atr":"(-1 * ts_mean((high - low)/(close + 0.001), 20))",
 "detr40":"(-1 * (close - ts_mean(close, 40)) / (ts_std_dev(close, 40) + 0.001))",
 "dvolmom":"ts_delta(ts_rank(volume * close, 60), 10)",
 "rangep":"(-1 * ts_corr(high - low, ts_delay(high - low, 1), 20))",
 "cubemom":"(-1 * ts_mean(signed_power(ts_delta(close,1), 3), 15))",
}
DC={k:4 for k in SIG}
def selfcorr(aid):
    for _ in range(16):
