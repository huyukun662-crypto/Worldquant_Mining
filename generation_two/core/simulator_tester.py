...
class SimulatorTester:
    ...
    def submit_simulation(
        ...
        try:
            ...
            # When explicit start/end dates are supplied, drop testPeriod so
            # the platform uses the exact window. Strip empty optional fields.
            if settings_dict.get('startDate') and settings_dict.get('endDate'):
                settings_dict.pop('testPeriod', None)
            for k in ('startDate', 'endDate'):
                if not settings_dict.get(k):
